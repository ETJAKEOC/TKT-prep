...
apply_patches() {
	...
	if [[ "$_CPU_OPTIMIZE" =~ ^(yes|y)$ ]] && [ -f $_SCRIPT_DIR/patches/common/more-uarches.patch ]; then
	echo "Applying more-uarches.patch..."
	patch -Np1 < $_SCRIPT_DIR/patches/common/more-uarches.patch
		if [ $? -ne 0 ]; then
	...
	if [[ "$_OPT_LEVEL" != "O2" ]] && [ -f $_SCRIPT_DIR/patches/common/optimize-user.patch ]; then
		echo "Applying user-specified optimization level..."
		patch -Np1 < $_SCRIPT_DIR/patches/common/optimize-user.patch
			if [ $? -ne 0 ]; then
